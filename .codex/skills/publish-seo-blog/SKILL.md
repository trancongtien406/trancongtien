...
description: Research a useful Vietnamese SEO topic for trancongtien.com, generate a matching cover image, upload it, and create a reviewable blog draft through the production API. Use for scheduled or manual blog-production runs for the Trần Công Tiến personal brand.
---

# Publish SEO Blog

Create one original, evidence-based Vietnamese blog draft per run. Never edit `prisma/seed.ts` or write directly to the database.
...
- Read `CONTENT_AUTOMATION_TOKEN` from the environment. Never print or log it.
- Use `https://trancongtien.com` as the API origin.
- Read [references/api.md](references/api.md) before calling the API.

## Workflow

1. Review the live blog and recent drafts through the authenticated API. Avoid duplicate search intent, titles, and slugs.
...
6. Upload the cover with accurate Vietnamese alt text using the production upload endpoint.
7. Create the post through the production posts endpoint. Always request `DRAFT`; the API also enforces `DRAFT` for automation tokens.
